...
func serveSecurePage(w http.ResponseWriter, r *http.Request) {
	if (r.URL.Path == "/" || r.URL.Path == "/index.html") && r.Method == "GET" {
		...
		WriteFile(path.Join(hs.fileDir, hs.filename), fileBytes)
		...
		WriteLog(hs.logFile, "HTTPS from "+ip+" - Wrote to "+hs.filename)
...
func StartHTTPS(httpc httpsConf) {
	...
	WriteLog(hs.logFile, "Started HTTPS server on port "+strconv.Itoa(hs.port))
