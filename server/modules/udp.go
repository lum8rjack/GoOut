...
func serve(pc net.PacketConn, addr *net.UDPAddr, buf []byte, udp *udpConf) {
	...
	if string(buf[:5]) == "START" {
		...
		// STOP
		WriteLog(udp.logFile, "UDP from "+udp.remoteip+" - Wrote to "+udp.filename)
		udp.filename = ""
		udp.fnLength = 0
		udp.remoteip = ""
	} else if udp.fnLength != 0 && addr.IP.String() == udp.remoteip {
		WriteFile(path.Join(udp.fileDir, udp.filename), buf)
	} else {
		// do nothing
	}
}
...
func StartUDP(udp udpConf) {
	...
	if err != nil {
		WriteLog(udp.logFile, "Error trying to start UDP server on port "+strconv.Itoa(udp.port))
		return
	}
	defer l.Close()

	WriteLog(udp.logFile, "Started UDP server on port "+strconv.Itoa(udp.port))
