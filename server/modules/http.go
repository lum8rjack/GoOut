...
func servePage(w http.ResponseWriter, r *http.Request) {
	if (r.URL.Path == "/" || r.URL.Path == "/index.html") && r.Method == "GET" {
		...
		WriteFile(path.Join(hc.fileDir, filename), uDec)
		//http.ServeFile(w, r, "config/http/index.html")
		...
		WriteFile(path.Join(hc.fileDir, handler.Filename), fileBytes)
		...
		WriteLog(hc.logFile, "HTTP from "+ip+" - Wrote to "+handler.Filename)
...
func StartHTTP(httpc httpConf) {
	...
	WriteLog(hc.logFile, "Started HTTP server on port "+strconv.Itoa(hc.port))
