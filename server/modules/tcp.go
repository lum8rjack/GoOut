...
func handleConnection(c net.Conn, tcp *tcpConf) {
	...
	if string(start[:5]) == "START" {
		...
		WriteLog(tcp.logFile, "TCP from "+strings.Split(rm, ":")[0]+" - Wrote to "+tcp.filename)
		for {
			n, err := c.Read(buf)
			if err != nil {
				//fmt.Println(err) //Prints EOF
			...
			if tcp.fnLength != 0 {
				WriteFile(path.Join(tcp.fileDir, tcp.filename), buf[:n])
			}
...
func StartTCP(tcp tcpConf) {
	...
	if err != nil {
		WriteLog(tcp.logFile, "Error trying to start TCP server on port "+strconv.Itoa(tcp.port))
		return
	}
	defer l.Close()

	WriteLog(tcp.logFile, "Started TCP server on port "+strconv.Itoa(tcp.port))
